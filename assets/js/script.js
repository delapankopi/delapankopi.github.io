        ...
        document.addEventListener("DOMContentLoaded", () => {
            ...
            // ===== PERBAIKAN: Auto close navbar di mobile (TERMASUK dropdown item) =====
            const navbarCollapse = document.getElementById('navbarNav');
            const navbarToggler = document.querySelector('.navbar-toggler');
            
            // Semua link termasuk dropdown item, KECUALI dropdown toggle
            const allNavLinks = document.querySelectorAll('.navbar-nav .nav-link, .navbar-nav .dropdown-item');
            
            allNavLinks.forEach(link => {
                link.addEventListener('click', function(e) {
                    // Abaikan jika ini adalah dropdown toggle (yang membuka dropdown)
                    if (this.getAttribute('data-bs-toggle') === 'dropdown') return;
                    
                    // Tutup navbar hanya di mobile
            ...
            fetch('content.json')
                ...
                .then(data => {
                    ...
                    if (siteConfig.hero) {
                        if (siteConfig.hero.background_image) {
    ...
    if (heroSection) {
        heroSection.style.background = `linear-gradient(rgba(0, 0, 0, 0.7), rgba(0, 0, 0, 0.7)), url('${siteConfig.hero.background_image}') center/cover no-repeat`;
                        ...
                        if (siteConfig.hero.hero_image) {
                            const heroImgEl = document.getElementById('heroMainImage');
                            if (heroImgEl) heroImgEl.src = siteConfig.hero.hero_image;
                        }
                        if (siteConfig.hero.heading) {
                            const heroHeading = document.getElementById('heroHeading');
                            if (heroHeading) heroHeading.innerText = siteConfig.hero.heading;
                        }
                        if (siteConfig.hero.subtitle) {
                    ...
                    if (siteConfig.brand && document.getElementById('navBrand')) document.getElementById('navBrand').innerText = siteConfig.brand;
                    if (siteConfig.tagline && document.getElementById('navTagline')) document.getElementById('navTagline').innerText = siteConfig.tagline;
                    if (siteConfig.address && document.getElementById('footerAddress')) document.getElementById('footerAddress').innerHTML = `<i class="fa-solid fa-location-dot me-2 text-warning"></i> ${siteConfig.address}`;
                    ...
                    if (document.getElementById('floatingWaBtn')) document.getElementById('floatingWaBtn').href = `${waFormatted}?text=${encodeURIComponent(floatingMsg)}`;
                    if (document.getElementById('footerWa')) {
                        document.getElementById('footerWa').href = waFormatted;
                        document.getElementById('footerWa').innerHTML = `<i class="fa-brands fa-whatsapp me-2 text-success"></i> +${siteConfig.whatsapp || '6285786012464'}`;
                    }
                    if (document.getElementById('socialWa')) document.getElementById('socialWa').href = waFormatted;
                    ...
                    if (document.getElementById('jarBtn8L')) document.getElementById('jarBtn8L').href = `${waFormatted}?text=${encodeURIComponent(jar8Msg)}`;
                    ...
                    if (document.getElementById('jarBtn16L')) document.getElementById('jarBtn16L').href = `${waFormatted}?text=${encodeURIComponent(jar16Msg)}`;

                    if (siteConfig.instagram && document.getElementById('footerIg')) document.getElementById('footerIg').href = siteConfig.instagram;
                    if (siteConfig.facebook && document.getElementById('footerFb')) document.getElementById('footerFb').href = siteConfig.facebook;
                    if (siteConfig.tiktok && document.getElementById('footerTiktok')) document.getElementById('footerTiktok').href = siteConfig.tiktok;
        ...
        // ===== PERBAIKAN: Deskripsi best seller FULL tanpa potongan =====
        function updateHeroBestSeller() {
            ...
            const heroNameEl = document.querySelector('#beranda .card .font-serif.fw-bold.text-white.fs-6');
            if (heroNameEl && bestSeller.name) {
                heroNameEl.textContent = bestSeller.name;
            }

            const heroDescEl = document.querySelector('#beranda .card .text-white-50.small.mb-0');
            if (heroDescEl && bestSeller.description) {
                // Tampilkan FULL tanpa dipotong
                heroDescEl.textContent = bestSeller.description;
            }

            const badgeEl = document.querySelector('#beranda .card .badge.bg-success.bg-opacity-25');
            if (badgeEl) {
                badgeEl.textContent = '★ Best Seller';
            }
        }

        /* Render Produk dengan Desain Kartu Slider Baru yang Modern */
